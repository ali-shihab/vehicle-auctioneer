...
public class Person implements Comparable<Person> {
  ...
  /** Regular expression for surname to match. */
  private String regEx = "[A-Z]{1}[a-z]*";
  /**
   ...
   */
  public Person(String forename, String surname, int age) throws InvalidAgeException, InvalidSurnameException {
	  super();
	  this.forename = forename;
	  
	  if (!surname.matches(regEx)) {
		  throw new InvalidSurnameException("Not a valid surname.");
	  }
	  this.surname = surname;
	  
	  if (age <= 0) {
		  throw new InvalidAgeException("Age too young.");
	  }
	  this.age = age; 
  }
